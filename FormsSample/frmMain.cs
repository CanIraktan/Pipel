...
namespace FormsSample
{
    public partial class frmMain : Form
    {
        ...
        private void PipeMessageReceived(PipelMessage message)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        List<int> sampleObj = cm.ConvertLoad<List<int>>();
                        txtMessages.AppendText(DateTime.Now.ToLongTimeString() + ":(Object) " + sampleObj + Environment.NewLine);
                    }
        ...
        {
            //PipeEngine.Send("Test Message", "other_app");
        ...
        private void Read_Infos()
        {
            try
            {
                ...
                {
                    string[] bilgiler = File.ReadAllText(InfosFileName).Split(';');

                    if(bilgiler.Length == 2)
                    {
                        txtName.Text = bilgiler[0];
                        txtReceiverName.Text = bilgiler[1];
                    }
                }
            }
            catch
